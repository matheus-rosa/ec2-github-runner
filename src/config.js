...
class Config {
  constructor() {
    ...
    if (this.input.mode === 'start') {
      const usingLaunchTemplate = !!this.input.launchTemplateName;

      if (!usingLaunchTemplate) {
        if (
          ...
          throw new Error(
              `Not all the required inputs are provided for the 'start' mode without a launch template`
          );
        }
      }
